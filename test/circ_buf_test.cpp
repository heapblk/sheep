...
void init(List &l) { l.head = l.tail = nullptr; }
...
TEST(CircBuf, Performance) {
  // push_back
  // ---------------------------------------------------------------------------------------------------------------------
  CircBuf<int> _v_circ_buf(100000);
  ...
  auto _circ_insert_time_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(_circ_end -
                                                           _circ_start)
          .count();
  ...
  auto _linked_insert_time_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(_linked_end -
                                                           _linked_start)
          .count();
